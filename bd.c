...
void crearTablas(sqlite3 *db) {
	...
	char sql1[] =
			"create table usuario(id int, nombre varchar(20), contrasenya varchar(20), precio float)";
	char sql2[] =
			"create table admin(nUsuario varchar(30), password varchar(30))";
	char sql3[] =
			"create table prenda(id int, tipo varchar(80), talla int, precio float )";
	//creo que esta sobra no? 
	// igual poner algo de carrito o así
	char sql4[] =
			"create table venta(dia int, mes varchar(15), anyo int, usu varchar(20), codarti varchar(20))";
...
int verPrendas(sqlite3 *db) {
	...
	char sql[] = "select id, tipo, talla, precio from prenda";
	...
	printf("Mostrando prendas disponibles:\n");
	do {
		result = sqlite3_step(stmt);
		if (result == SQLITE_ROW) {
			id = sqlite3_column_int(stmt, 0);
			strcpy(tipo, (char*) sqlite3_column_text(stmt, 1));
			talla = sqlite3_column_int(stmt, 2);
			precio = sqlite3_column_int(stmt, 3);
			printf("ID: %d tipo: %s talla: %d precio: %.2f\n", id, tipo, talla, precio);
		}
...
void cambiarContrasenaUsuario(sqlite3 *db, char *nombre, char *contrasenya) {
	...
	sprintf(sql, "update usuario set contrasenya='%s' where nombre='%s'",
			contrasenya, nombre);
	sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	sqlite3_step(stmt);
	sqlite3_finalize(stmt);
...
void cambiarContrasenaAdmin(sqlite3 *db, char *nUsuario, char *password) {
	...
	sprintf(sql, "update admin set password='%s' where nUsuario='%s'", password,
			nUsuario);
	sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	sqlite3_step(stmt);
	sqlite3_finalize(stmt);
...
void anyadirPrenda(sqlite3 *db, int *id, char *tipo, int talla, float precio) {
	...
	sprintf(sql, "insert into prenda values('%i', '%s', %i, %2.f)", id, tipo,
			talla, precio);
	sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	sqlite3_step(stmt);
	sqlite3_finalize(stmt);
